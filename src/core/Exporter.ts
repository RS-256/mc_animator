...
async function cleanupFFmpegDir(ffmpeg: FFmpeg, dir: string): Promise<void> {
  ...
  try {
    ...
    await Promise.all(
      entries
        .filter(entry => !entry.isDir)
        .map(entry => ffmpeg.deleteFile(`${dir}/${entry.name}`)),
    )
...
async function exportH265Mp4WithWebCodecs(
  ...
  if (!support) {
    throw new Error('このブラウザは現在の設定で H.265 WebCodecs エンコードに対応していません。H.265 は環境依存が強いため、PNGシーケンスからローカル ffmpeg で変換してください。')
  }
...
async function exportEncodedVideo(
  ...
  try {
    ...
    try {
      await execWithLogs(
        ffmpeg,
        preset.video.ffmpegArgs(fps, inputPattern, outputPath),
        preset.label,
      )
    } catch (error) {
