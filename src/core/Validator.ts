import type { AnimationSchema, ValidationResult, ValidationMessage } from "../types/schema"
...
export function validate( data: unknown ): ValidationResult {
  ...
  if ( fps > 0 && tps > 0 && durationTicks > 0 ) {
    ...
    if ( totalFrames > MAX_FRAMES ) {
      msgs.push(
        validationMessage( "error", "validation.frameLimitExceeded", {
          totalFrames,
    ...
    if ( ! Array.isArray( obj.keyframes ) || obj.keyframes.length === 0 ) {
      ...
      for ( let k = 0; k < obj.keyframes.length; k++ ) {
        ...
        if (
          ...
          msgs.push(
            validationMessage( "error", "validation.easingInvalid", {
              id,
              index: k,
        ...
        if ( obj.type === "block" && keyframe.multiplier !== undefined ) {
          ...
          if ( typeof multiplier !== "number" || ! Number.isFinite( multiplier ) || multiplier < 0 ) {
            msgs.push(
              validationMessage( "error", "validation.multiplierInvalid", {
                id,
                index: k
              } )
            )
        ...
        if ( obj.type === "camera" ) {
          ...
          if (
            ...
            msgs.push(
              validationMessage( "error", "validation.cameraPathInvalid", {
                id,
                index: k,
  ...
  if ( msgs.filter( ( m ) => m.severity === "error" ).length === 0 ) {
    const activeCam = typeof meta.active_camera === "string" ? meta.active_camera : DEFAULT_CAMERA_ID
    ...
    if ( cameraIds.length > 0 && ! ids.has( activeCam ) ) {
      ...
      msgs.push(
        validationMessage( "warning", "validation.defaultCameraMissing", {
...
function hasRelativeComponent( value: unknown ): boolean {
  return (
    Array.isArray( value ) &&
    value.some( ( component ) => typeof component === "string" && component.startsWith( "~" ) )
  )
}

function validateCameraRelativePositions( id: string, rawKeyframes: unknown, msgs: ValidationMessage[] ) {
  ...
  const keyframes = rawKeyframes
    .map( ( kf, index ) => {
      ...
      const absoluteTick = keyframe.tick_mode === "relative" ? lastAbsoluteTick + keyframe.tick : keyframe.tick
