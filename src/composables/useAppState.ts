...
const totalFrames = computed( () => Math.ceil( ( totalTicks.value * fps.value ) / tps.value ) )

const cameraObjects = computed( () => schema.value?.objects.filter( ( o ) => o.type === "camera" ) ?? [] )

const blockObjects = computed( () => schema.value?.objects.filter( ( o ) => o.type === "block" ) ?? [] )
...
function updateKeyframe( objectId: string, index: number, patch: Partial< BlockKeyframe | CameraKeyframe > ) {
  if ( ! schema.value ) return
...
function reorderKeyframe( objectId: string, fromIndex: number, toIndex: number ) {
  ...
  const objects = schema.value.objects.map( ( object ) => {
    if ( object.id !== objectId ) return object
    if ( fromIndex < 0 || toIndex < 0 || fromIndex >= object.keyframes.length || toIndex >= object.keyframes.length ) {
      return object
    }
