...
interface ModelElement {
  ...
  faces?: Record<
    string,
    { texture: string; cullface?: string; uv?: [ number, number, number, number ]; rotation?: number }
  >
}
...
async function fetchModelMerged( modelPath: string, loader: IAssetLoader, depth = 0 ): Promise< ModelJson > {
  if ( depth > 8 ) return {}
...
function buildFaceMapFromModel( model: ModelJson ): Partial< Record< BoxFace, FaceTextureInfo > > | null {
  ...
  if ( top || side || bottom || end ) {
    ...
    if ( t ) {
      partial.up = { texture: t }
    }
    if ( b ) {
      partial.down = { texture: b }
    }
    if ( s ) {
...
function rotateUvCorners( corners: [ number, number ][], rotation: number ): [ number, number ][] {
...
function applyFaceUvs( geometry: THREE.BoxGeometry, faceMap: Partial< Record< BoxFace, FaceTextureInfo > > ) {
...
async function buildFlatItemObject( model: ModelJson, loader: IAssetLoader ): Promise< THREE.Object3D | null > {
  const textures = model.textures ?? {}
...
export async function buildBlockMeshData(
  ...
  // 2. model JSON を再帰取得
  const model = modelName ? await fetchModelMerged( modelName, loader ) : { textures: { all: `block/${ blockName }` } }
